...
HRESULT LogAttributeValueByIndex(IMFAttributes *pAttr, DWORD index)
{
    ...
    GuidName = GetGUIDNameConst(guid);
    ...
    if (hr == S_FALSE)
    {
        ...
        {
        case VT_UI4:
            qDebug() << "Item: " << GuidName << " Value: " << var.ulVal;
            break;

        case VT_UI8:
            qDebug() << "Item: " << GuidName << " Value: " << "!!var.uhVal!!";
            break;

        case VT_R8:
            qDebug() << "Item: " << GuidName << " Value: " << var.dblVal;
            break;

        case VT_CLSID:
            GuidValName = GetGUIDNameConst(*var.puuid);
            qDebug() << "Item: " << GuidName << " Value: " << GuidValName;
            break;

        case VT_LPWSTR:
            qDebug() << "Item: " << GuidName << " Value: " << var.pwszVal;
            break;

        case VT_VECTOR | VT_UI1:
            qDebug() << "Item: " << GuidName << " Value: " << "!!byte array!!";
            break;

        case VT_UNKNOWN:
            qDebug() << "Item: " << GuidName << " Value: " << "IUnknown";
            break;

        default:
            qDebug() << "Item: " << GuidName << " Value: " << "!!Unexpected attribute type (vt = " << var.vt << ")!!";
            break;
        }
    }
...
HRESULT GetGUIDName(const GUID& guid, WCHAR **ppwsz)
{
    ...
    QString pcwsz = GetGUIDNameConst(guid);
    if (pcwsz.isEmpty())
    {
        size_t cchLength = 0;

        hr = StringCchLength((STRSAFE_PCNZCH)pcwsz, STRSAFE_MAX_CCH, &cchLength);
        if (FAILED(hr))
        {
            goto done;
        }
        ...
        hr = StringCchCopy((STRSAFE_LPSTR)pName, cchLength + 1, (STRSAFE_LPCSTR)pcwsz);
        if (FAILED(hr))
        {
            goto done;
        }
...
HRESULT LogVideoArea(const PROPVARIANT& var)
{
    ...
    qDebug() << "(" << OffsetToFloat(pArea->OffsetX) << "," << OffsetToFloat(pArea->OffsetY) << ") ("
...
void DBGMSG(PCWSTR format, ...)
{
    ...
    if (SUCCEEDED(StringCbVPrintf((STRSAFE_LPSTR)msg, sizeof(msg), (STRSAFE_LPCSTR)format, args)))
    {
